...
const { sword, wtype, prefix, sleep, extractUrlsFromString, fb, ytaudio, config, ytvideo, xdl, tt, insta, mediaFire, rand, getBuffer } = require("../core")
...
cmd: "apk",
  ...
  try {
    ...
    var data = await m.axios(`https://sword-api.vercel.app/apk?q=${text}`)
...
cmd: "apksearch",
  ...
  try {
    ...
    var data = await m.axios(`https://api.swordai.biz.id/apkdl?id=${q}`)
    if (data.error) return await m.send("_Apk not found.._")
    ...
    var info = await m.axios(`https://api.swordai.biz.id/apksearch?query=${text}`);
...
sword({
  ...
  let data;
  try {
    data = await m.axios(`https://sword-api.vercel.app/subtitle?q=${text}`);
  } catch (e) {
...
cmd: "subtitlesearch|subtitles",
  ...
  try {
    ...
    let data = await m.axios(`https://sword-api.vercel.app/subtiledl?q=${encodeURIComponent(pageUrl)}`);
    ...
    const info = await m.axios(`https://sword-api.vercel.app/subtitlepage?q=${text}`);
...
sword({
        ...
        m.react("⏰")
        var ytsr = await yts(text)
        if (!ytsr.videos || ytsr.videos.length === 0) return await m.send("_*No results found for your query*_")
        
        var ytsa = ytsr.videos[0]
        var dl = await ytaudio(ytsa.url)
        
        if (!dl || !dl.url) return await m.send("_*Failed to download audio*_")
        
        var cap = `00:00 ───◁ㅤ ❚❚ ㅤ▷─── ${ytsa.duration.timestamp} ♡`
        m.react("✅")
        
        return await m.send(dl.url, {
            ptt: false,
            mimetype: 'audio/mpeg',
            contextInfo: {
              externalAdReply: {
        ...
        console.error(err)
        return await m.send(`_*Error:*_ ${err.message}`)
        }
});
...
sword({
        ...
        try {
                ...
                const ttregex = /https:\/\/(?:www\.|vm\.|m\.|vt\.)?tiktok\.com\/(?:(@[\w.-]+\/(?:video|photo)\/\d+)|v\/\d+\.html|[\w-]+\/?)(?:\?.*)?$/
                var link = links.find(url => ttregex.test(url));
                const dta = await fetch(`https://sword-api.vercel.app/tik-img?url=${encodeURIComponent(link)}`);
