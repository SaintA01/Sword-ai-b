...
const {sword, webp2png, webp2mp4, elevenlabs, rand, getBuffer, toAudio, config, processAudio, extractUrlsFromString, toPTT, isMediaURL, wtype} = require("../core")
...
cmd: "black",
  ...
  try {
    ...
    const blackImg = await getBuffer("https://cdn.swordai.biz.id/serve/n2BwUtItyeae.jpg")
...
cmd: "roundstk|round",
  ...
  try {
    ...
    categories: ["🤩", "🎉"],
    id: "https://github.com/M3264/Sword Bot",
    quality: 75,
    })
...
sword({
  ...
  if (m.quoted.sticker) {
    ...
    if (text) {
      ...
      data = config().AUDIO_DATA.split(";") || ["sword", "v2", ""]
    }
    
    const title = data[0] || "sword"
    const artist = data[1] || "v2"
    const coverUrl = data[2] || "https://cdn.swordai.biz.id/serve/tuNyPANPYD2v.png"
    
    try {
      const audioBase64 = audioResult.toString('base64')
      
      const response = await fetch('https://sword-api.vercel.app/add-mp3-meta', {
        method: 'POST',
        headers: {
