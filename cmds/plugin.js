...
const { sword, wtype, storeData, getData, isUrl, extractUrlsFromString, sleep } = require("../core")
...
const arr = await extractUrlsFromString(input)
  ...
  for (const element of arr) {
    ...
    try {
      ...
      } else if (parsed.host === "plugins.sword-ai.web.id") {
    ...
    try {
      ...
      const isEvent = data.includes("sword({") && data.includes("on:")
      const pluginMeta = {
...
sword({
  ...
  let norm = input
if (norm.includes("plugins.sword-ai.web.id") && !norm.includes("/raw")) {
